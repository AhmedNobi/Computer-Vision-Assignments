...
class PanoramaGenerator:
    def generate_panorama_image(self, images, ratio=0.75, reprojThresh=4.0, showMatches=False):
        ...
        result = cv2.warpPerspective(imageA, H, (800, 200))
        # End Student Code
    ...
    def detectAndDescribe(self, image):
        ...
        kps, features = sift_model.detectAndCompute(image, None)
        # End Student Code
    ...
    def matchKeypoints(self, kpsA, kpsB, featuresA, featuresB, ratio, reprojThresh):
        ...
        if len(matches) > 4:
            ...
            ptsA = np.float32([kpsA[m[1]] for m in matches])
            ptsB = np.float32([kpsB[m[0]] for m in matches])
            # End Student Code
